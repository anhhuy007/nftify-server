...
class UserService {
    ...
    async getStampsIDsByOwner(userId) {
        ...
        const latestOwnerships = ownerships.reduce((acc, ownership) => {
            ...
            if (
                !existingOwnership ||
                new Date(ownership.timestamp) >
                    new Date(existingOwnership.timestamp)
            ) {
    ...
    async editStamp(stamp) {
      ...
      try{
          ...
          if (stamp.isListed === true || stamp.isListed === false) {
            const newItemInsight = await itemInsightModel.findOneAndUpdate({itemId : stamp._id}, {
              isListed: stamp.isListed,
              updatedAt: new Date()
            }, { new: true });
          ...
          if (stamp.collection.id) {
              const collectionWithStamp = await collectionService.addStampToCollection(
                  stamp.collection.id,
                  stamp._id
              );
